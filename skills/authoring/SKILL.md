...
| "I'll add the missing `version_constraint` and leave the `${data[...]}` paths alone." | WRONG. The pin decides the path shape. Unpinned is `DeviceQuery.Device.query.devices`; `~1` is `DeviceQuery.devices`. Adding the pin without shortening the paths fails at release with `unknown variable`. Do both in one edit. |

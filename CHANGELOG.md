...
- **`validate.py` recognizes `summary` as a valid top-level key.** Fusion supports a console execution-summary template (`summary: |- ${data['WorkflowCustomVariable.workflow-result']}...`) — confirmed against 39 already-shipped production workflows in `fusion-workflows` across multiple packages — but the validator's top-level allowlist didn't include it, so every one of those workflows was rejected with `Unknown top-level key(s): summary` even though nothing was actually wrong with them. `summary` is now accepted alongside the other console-export keys (`disconnected_nodes`, `output_fields`).
